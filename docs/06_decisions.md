...
## Decision 024: 初回の実ProviderはOllama native `/api/generate` を最小設定で呼ぶ

- Status: Accepted
- Phase: 6
- Decision: `OllamaLlmProvider` は native Ollama API の `POST /api/generate` を使用し、初期設定では `base_url="http://127.0.0.1:11434"`、`stream=False` を固定する。model は CLI の `--llm-model` から指定可能とし、既定値は `gemma4:latest` にする。
- Rationale: 実機環境で利用可能な model 名は差異があるため、base_url まで同時に広げずに model だけを先に外出しする。これで adapter 境界を保ったまま、CLI 疎通失敗の主要因を除ける。
- Details: prompt組み立ては `LlmPromptBuilder`、レスポンス変換は `LlmResponseParser` に委譲する。HTTP timeout は provider が持ち、HTTP失敗は呼び出し側へ返す。`none` と `fake` は `--llm-model` を無視する。
